...
#=
##############################################################################
# exchange_dim! – full, self-contained version for MPI.jl v0.20.x
#   • NC‐first, column-major array layout  (NC, X, Y, …)
#   • variable ghost width  `ls.nw`
#   • per-direction phase factors in `ls.phases`
#   • halo width sent as an MPI derived datatype stored in `ls.types`
#   • works on CPU or CUDA/ROCM GPU memory (JACC provides the arrays)
##############################################################################
function exchange_dim!(ls::Lattice{D,T,TA,T1}, d::Int) where {D,T,TA,T1}
    bufM, bufP        = ls.buf[2d-1], ls.buf[2d]         # work buffers
    rankM, rankP      = ls.nbr[d]
    me                = ls.myrank
    reqs              = MPI.Request[]

    baseT = MPI.Datatype(T)                              # elementary type
    ...
    if rankM == me
        ...
        cnt = length(bufM)
        sendB = MPI.Buffer(bufM)             # contiguous buffer
        recvB = MPI.Buffer(bufM)
        push!(reqs, MPI.Isend(sendB, rankM, d,    ls.cart))
        push!(reqs, MPI.Irecv!(recvB, rankM, d+D, ls.cart))
    end
    ...
    if rankP == me
        ...
        cnt = length(bufP)
        sendB = MPI.Buffer(bufP)
        recvB = MPI.Buffer(bufP)
        push!(reqs, MPI.Isend(sendB, rankP, d+D, ls.cart))
        push!(reqs, MPI.Irecv!(recvB, rankP, d,   ls.cart))
    end
...
function exchange_dim!(ls::Lattice{D,T,TA,T1}, d::Int) where {D,T,TA,T1}
    ...
    if rankM == myrank
        ...
        if wrap
            ...
            _mul_phase!(bufM, ls.phases[d])
            buf = MPI.Buffer(bufM,count,)
            push!(req, MPI.Isend(buf, rankM, d, ls.cart))
        else
            buf = MPI.Buffer(ls.A,ls.types[(d,:minus)])
            push!(req, MPI.Isend(buf, rankM, d, ls.cart;
                                 datatype = ls.types[(d,:minus)]))
        end
        push!(req, MPI.Irecv!(bufM, rankM, d+D, ls.cart;
                              datatype = ls.types[(d,:plus)]))
    end
